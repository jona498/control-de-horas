...
⢀⣾⠃⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢿⠀⠀⠀⠀⠀⠀⠀⠀⠀
    ...
    Gestión de Horas v4 - Programado por Jonathan Rodriguez
    `);
    ...
    const imputeAllBtn = document.getElementById('impute-all-btn'); 
    ...
    const appVersionTrigger = document.getElementById('app-version-trigger'); // ¡NUEVO!
    const easterEggImage = document.getElementById('easter-egg-image'); // ¡NUEVO!

    // ¡NUEVAS REFERENCIAS! Vista Horas Extra
    const navOvertimeBtn = document.getElementById('nav-overtime-btn');
    const overtimeView = document.getElementById('overtime-view');
    const overtimeAddForm = document.getElementById('overtime-add-form');
    const overtimeDateInput = document.getElementById('overtime-date-input');
    const overtimeHoursInput = document.getElementById('overtime-hours-input');
    const overtimeTypeSelect = document.getElementById('overtime-type-select');
    const overtimeRedemptionSelect = document.getElementById('overtime-redemption-select');
    const overtimeDescInput = document.getElementById('overtime-desc-input');
    const overtimeSummaryMoney = document.getElementById('overtime-summary-money');
    const overtimeSummaryFreeHours = document.getElementById('overtime-summary-free-hours');
    const overtimePendingListContainer = document.getElementById('overtime-pending-list-container');
    const overtimeRedeemedListContainer = document.getElementById('overtime-redeemed-list-container');
    ...
    let allOvertime = {}; // ¡NUEVO ESTADO!
    let projectChartInstance = null;
    let jpChartInstance = null;

    // --- FUNCIONES DE ALMACENAMIENTO (¡ACTUALIZADO!) ---
    function saveOvertimeToStorage() { localStorage.setItem('myOvertime', JSON.stringify(allOvertime)); } // ¡NUEVO!
    ...
    function loadFromStorage() {
        ...
        const overtimeJSON = localStorage.getItem('myOvertime'); // ¡NUEVO!
        if (overtimeJSON) allOvertime = JSON.parse(overtimeJSON); // ¡NUEVO!
    }
    ...
    function saveAllDataToStorage() {
        ...
        saveOvertimeToStorage(); // ¡NUEVO!
    }
    
    // --- FUNCIÓN DE LÓGICA DE DÍA (Sin cambios) ---
    function getEffectiveDayType(dateString) {
        const storedType = dayTypes[dateString];
        if (storedType) {
            return storedType;
        }
    ...
    // --- FUNCIONES DE CONFIGURACIÓN Y BACKUP (¡ACTUALIZADO!) ---
    ...
    function prepareBackupData() {
        return { settings, allTasks, dayTypes, projectSuggestions, jpSuggestions, allOvertime }; // ¡ACTUALIZADO!
    }
    ...
    function handleFileSelected(event) {
        ...
        reader.onload = (e) => {
            try {
                ...
                allOvertime = importedData.allOvertime || {}; // ¡NUEVO!
    ...
    function navigateTo(view) {
        ...
        overtimeView.classList.remove('active'); // ¡NUEVO!
        navCalendarBtn.classList.remove('active');
        navSummaryBtn.classList.remove('active');
        navSettingsBtn.classList.remove('active');
        navOvertimeBtn.classList.remove('active'); // ¡NUEVO!
        ...
        if (view === 'calendar') {
            ...
            loadSettingsIntoUI();
            renderSuggestionManagers();
        } else if (view === 'overtime') { // ¡NUEVO BLOQUE!
            overtimeView.classList.add('active');
            navOvertimeBtn.classList.add('active');
            renderOvertimeView(); // ¡Nueva función!
        }
    }
    ...
    function handleGenerateSummary() {
        ...
        renderCharts(projectData, jpData);
    ...
    function renderCharts(projectData, jpData) {
        ...
        projectChartInstance = new Chart(projectChartCtx, {
            ...
            options: { responsive: true, legend: { position: 'top', labels: { color: chartTextColor } } }
        });
    ...
    function renderCalendar(year, month) {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const storedType = dayTypes[fullDate];
            if (storedType === 'vacaciones') { dayCell.classList.add('vacaciones'); } 
            else if (storedType === 'festivo') { dayCell.classList.add('festivo'); }
            dayCell.innerHTML = `
    ...
    // --- ¡MODIFICADA! --- (Lógica para imputeAllBtn)
    function selectDay(clickedCell, dateString) {
        ...
        imputeAllBtn.style.display = 'none'; // Ocultar por defecto
        
        const effectiveDayType = getEffectiveDayType(dateString);

        if (effectiveDayType === 'festivo' || effectiveDayType === 'vacaciones') {
            const typeText = effectiveDayType === 'festivo' ? 'Día Festivo' : 'Día de Vacaciones';
            taskList.innerHTML = `<p style="color: #888; text-align: center; padding: 20px 0;">${typeText}</p>`;
            addTaskForm.style.display = 'none';
        } else { 
            addTaskForm.style.display = 'block';
            imputeAllBtn.style.display = 'block'; // Mostrar
    ...
    function updateSidebarSummary(dateString) {
        ...
        const expectedHours = (effectiveDayType === 'workday') ? defaultHours : 0.0;
        totalHoursDisplay.textContent = `${totalHours.toFixed(1)}h / ${expectedHours.toFixed(1)}h`;
    ...
    function updateDaySummaryInGrid(dateString) {
        ...
        } else if (effectiveDayType === 'vacaciones') {
            daySummaryEl.textContent = 'Vacaciones';
            daySummaryEl.classList.add('complete');
        } else if (effectiveDayType === 'workday') {
            if (totalHours === 0) {
    ...
    function setDayType(type) {
        ...
        if (type === 'workday' && !isWeekend) {
            delete dayTypes[selectedDate];
        } else {
        ...
        if (selectedCell) {
            ...
            if (dayTypes[selectedDate] === 'vacaciones' || dayTypes[selectedDate] === 'festivo') {
                selectedCell.classList.add(dayTypes[selectedDate]);
            }
            updateDaySummaryInGrid(selectedDate);
    ...
    /**
     * Marca todas las tareas del día seleccionado como imputadas.
     */
    function handleImputeAll() {
        if (!selectedDate) return;

        const tasksForDay = allTasks[selectedDate];
        if (!tasksForDay || tasksForDay.length === 0) {
            alert('No hay tareas para imputar en este día.');
            return;
        }
        ...
        if (!changesMade) {
            alert('Todas las tareas ya estaban imputadas.');
            return;
        }
    ...
    // --- ¡NUEVAS FUNCIONES DE HORAS EXTRA! ---

    /**
     * Define las reglas de canjeo del PDF.
     */
    const OVERTIME_RULES = {
        'A': { 'dinero': 12.0, 'horas_libres': 1.5 }, //
        'B': { 'dinero': 18.0, 'horas_libres': 2.0 }, //
        'C': { 'dinero': 24.0, 'horas_libres': 2.5 }  //
    };

    /**
     * Renderiza toda la vista de Horas Extra (resumen y listas).
     */
    function renderOvertimeView() {
        // 1. Calcular y mostrar el resumen
        const summary = calculateOvertimeSummary();
        overtimeSummaryMoney.textContent = `${summary.totalMoney.toFixed(2)} €`;
        overtimeSummaryFreeHours.textContent = `${summary.totalFreeHours.toFixed(1)} h`;
        
        // 2. Renderizar las listas
        renderOvertimeLists();
    }

    /**
     * Calcula los totales pendientes de canjear.
     * @returns {object} - Objeto con { totalMoney, totalFreeHours }
     */
    function calculateOvertimeSummary() {
        let totalMoney = 0;
        let totalFreeHours = 0;

        for (const date in allOvertime) {
            allOvertime[date].forEach(entry => {
                if (!entry.redeemed) {
                    const rules = OVERTIME_RULES[entry.type];
                    if (entry.redemption === 'dinero') {
                        totalMoney += rules.dinero * entry.hours;
                    } else if (entry.redemption === 'horas_libres') {
                        totalFreeHours += rules.horas_libres * entry.hours;
                    }
                }
            });
        }
        return { totalMoney, totalFreeHours };
    }

    /**
     * Dibuja las tablas de horas pendientes y canjeadas.
     */
    function renderOvertimeLists() {
        let pendingHtml = '<table class="overtime-list-table"><thead><tr><th>Fecha</th><th>Descripción</th><th>Horas</th><th>Tipo</th><th>Canje</th><th>Valor</th><th>Acciones</th></tr></thead><tbody>';
        let redeemedHtml = '<table class="overtime-list-table"><thead><tr><th>Fecha</th><th>Descripción</th><th>Horas</th><th>Tipo</th><th>Canje</th><th>Valor</th><th>Acciones</th></tr></thead><tbody>';
        ...
        redeemedEntries.sort((a, b) => b.date.localeCompare(a.date));

        // Construir HTML para pendientes
        if (pendingEntries.length === 0) {
            pendingHtml = "<p>No hay horas pendientes de canjear.</p>";
        } else {
            pendingEntries.forEach(entry => {
                const rules = OVERTIME_RULES[entry.type];
                let valueText = '';
                if (entry.redemption === 'dinero') {
                    valueText = `<strong>${(rules.dinero * entry.hours).toFixed(2)} €</strong>`;
                } else {
                    valueText = `<strong>${(rules.horas_libres * entry.hours).toFixed(1)} h</strong>`;
                }
                ...
                pendingHtml += `
                    <tr>
                        ...
                        <td>Tipo ${entry.type}</td>
                        <td>${entry.redemption === 'dinero' ? 'Dinero' : 'Horas Libres'}</td>
                        <td>${valueText}</td>
                        <td class="action-cell">
                            <button class="redeem-btn" data-id="${entry.id}">Canjear</button>
                            <button class="delete-overtime-btn" data-id="${entry.id}">&times;</button>
                        </td>
                    </tr>`;
            });
        ...
        // Construir HTML para canjeadas
        if (redeemedEntries.length === 0) {
            redeemedHtml = "<p>No hay horas en el historial.</p>";
        } else {
            redeemedEntries.forEach(entry => {
                const rules = OVERTIME_RULES[entry.type];
                let valueText = '';
                if (entry.redemption === 'dinero') {
                    valueText = `${(rules.dinero * entry.hours).toFixed(2)} €`;
                } else {
                    valueText = `${(rules.horas_libres * entry.hours).toFixed(1)} h`;
                }
            ...
            redeemedHtml += '</tbody></table>';
        }

        overtimePendingListContainer.innerHTML = pendingHtml;
        overtimeRedeemedListContainer.innerHTML = redeemedHtml;
    }

    /**
     * Maneja el envío del formulario de añadir horas extra.
     */
    function handleAddOvertime(event) {
        event.preventDefault(); // Evita que la página se recargue

        const date = overtimeDateInput.value;
        const hours = parseFloat(overtimeHoursInput.value);
        const type = overtimeTypeSelect.value;
        const redemption = overtimeRedemptionSelect.value;
        const description = overtimeDescInput.value.trim();

        if (!date || isNaN(hours) || hours <= 0 || !type || !redemption || !description) {
            alert('Por favor, rellena todos los campos correctamente.');
            return;
        }

        const newOvertimeEntry = {
            id: Date.now(),
            date: date,
            hours: hours,
            type: type,
        ...
        allOvertime[date].push(newOvertimeEntry);
        
        saveOvertimeToStorage();
        renderOvertimeView(); // Re-dibuja la vista completa

        // Limpia el formulario
        overtimeAddForm.reset();
        // Vuelve a poner la fecha de hoy por defecto si estaba vacía
        if (!overtimeDateInput.value) {
            overtimeDateInput.value = new Date().toISOString().split('T')[0];
        }
    }

    /**
     * Maneja los clics en los botones de las listas (Canjear / Borrar).
     */
    function handleOvertimeListClick(event) {
        const target = event.target;
        ...
        for (const date in allOvertime) {
            const entry = allOvertime[date].find(e => e.id === Number(entryId));
            if (entry) {
                entryFound = entry;
                dateOfEntry = date;
                break;
        ...
        if (target.classList.contains('delete-overtime-btn')) {
            if (!confirm(`¿Seguro que quieres borrar esta entrada de horas extra?\n\n${entryFound.description} (${entryFound.hours}h)`)) {
                return;
            }
            allOvertime[dateOfEntry] = allOvertime[dateOfEntry].filter(e => e.id !== Number(entryId));
            if (allOvertime[dateOfEntry].length === 0) {
                delete allOvertime[dateOfEntry];
            }
        }
        
        // Acción de CANJEAR
        if (target.classList.contains('redeem-btn')) {
            entryFound.redeemed = true;
        }

        saveOvertimeToStorage();
        renderOvertimeView(); // Re-dibuja la vista
    }

    // --- FIN DE FUNCIONES DE HORAS EXTRA ---
    ...
    navOvertimeBtn.addEventListener('click', () => navigateTo('overtime')); // ¡NUEVO!
    ...
    // Horas Extra
    overtimeAddForm.addEventListener('submit', handleAddOvertime); // ¡NUEVO!
    overtimePendingListContainer.addEventListener('click', handleOvertimeListClick); // ¡NUEVO!
    overtimeRedeemedListContainer.addEventListener('click', handleOvertimeListClick); // ¡NUEVO!
    ...
    imputeAllBtn.addEventListener('click', handleImputeAll); 
    ...
    // ¡NUEVO! Easter Egg Listener
    appVersionTrigger.addEventListener('click', () => {
        if (easterEggImage.style.display === 'none') {
    ...
    renderCalendar(currentDate.getFullYear(), currentDate.getMonth());
    navigateTo('calendar');
    overtimeDateInput.value = new Date().toISOString().split('T')[0]; // ¡NUEVO!
